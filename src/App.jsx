...
const isMockMode = [true, "true", "1", "yes"].includes(import.meta.env.VITE_MOCK_MODE);
...
function Header({ step, caseData, onHome }) {
  return (
    <header className="app-header">
      <button className="brand" onClick={onHome} aria-label="Case Validation home">
  ...
  return (
    <main className="landing">
      <section className="landing-card">
        ...
        <p>Review outstanding claim requirements and submit the completed fulfillment package.</p>
        <button className="primary-button" onClick={onStart}>Open Awaiting Fulfillment</button>
      </section>
    </main>
  );
}
...
function CaseList({ cases, onSelect, onBack }) {
  ...
  const filtered = cases.filter((item) => {
    const text = `${item.pxRefObjectInsName || ""} ${item.pyLabel || ""}`.toLowerCase();
    ...
    <main className="page-shell">
      <section className="list-card">
        <div className="section-heading">
          ...
          <button className="text-button" onClick={onBack}>Home</button>
        </div>
        <div className="list-toolbar">
          <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search case or beneficiary" />
        </div>
        <div className="case-list">
          {filtered.map((item) => {
            const id = item.pxRefObjectKey || item.pxRefObjectInsName;
            return (
              <button className="case-row" key={item.pzInsKey} onClick={() => onSelect(id)}>
...
function RequirementRow({ item, comment, onComment }) {
  return (
    <div className="requirement-row">
      ...
      <div className={`workbench-status ${item.workbenchStatus === "NIGO" ? "nigo" : ""}`}>{item.workbenchStatus}</div>
      <div className="comment-cell">
        <textarea value={comment} onChange={(event) => onComment(event.target.value)} aria-label={`${item.name} beneficiary comments`} />
      </div>
    </div>
  );
}
...
function CaseDetail({ caseData, onSubmit, onSave, onBack, submitting, onUpload, uploading, attachments }) {
  ...
  const requirements = caseData?.requirements?.length ? caseData.requirements : defaultRequirements;
  ...
  const claimType = contentValue(content, ["claimType", "ClaimType", "pyClaimType"], "Death Claim");
  const claimSubType = contentValue(content, ["claimSubType", "ClaimSubType", "pyClaimSubType"], "Regular");
  const claimSource = contentValue(content, ["claimSource", "ClaimSource", "pyClaimSource"], "Call");
  const deathDate = contentValue(content, ["dateOfDeath", "DateOfDeath", "pyDateOfDeath"], "05/01/2026");

  return (
    <main className="page-shell detail-page">
      <div className="detail-topline">
        <button className="text-button" onClick={onBack}>← Back</button>
        <div className="case-reference">{caseData?.businessID || caseData?.ID}</div>
      </div>

      <section className="case-information">
        <h1>Beneficiary Case Information</h1>
        <div className="information-grid">
          <div><span>Claim Type</span><strong>{claimType}</strong></div>
          <div><span>Claim Sub Type</span><strong>{claimSubType}</strong></div>
          <div><span>Claim Source</span><strong>{claimSource}</strong></div>
          <div><span>Date of Death</span><strong>{dateValue(deathDate, "05/01/2026")}</strong></div>
        </div>
      </section>
      ...
      <section className="fulfillment-section">
        ...
        <div className="requirements-table">
          ...
          {requirements.map((item) => (
            <RequirementRow
              ...
              onComment={(value) => setComments((current) => ({ ...current, [item.name]: value }))}
            />
          ))}
        </div>
        ...
        <div className="attachment-area">
          ...
          <button className="outline-button" onClick={chooseFile} disabled={uploading}>
            {uploading ? "UPLOADING…" : "ATTACH CONTENT"}
          </button>
          {attachments.length > 0 && (
            <div className="attachment-list">
              {attachments.map((attachment) => <span key={attachment.id}>{attachment.name}</span>)}
            </div>
          )}
        </div>
      </section>
...
export default function App() {
  ...
  const authenticate = useCallback(async () => {
    ...
    const data = await apiResponse(response, "Authentication failed — check credentials.");
    return data.access_token;
  }, []);

  const getCases = useCallback(async (accessToken) => {
    const response = await fetch(`${API_BASE}/data_views/D_GetCasesOnAssignment`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${accessToken}` },
      body: JSON.stringify({ dataViewParameters: { TaskLabel: "Awaiting Fulfillment" } }),
    });
  ...
  const openQueue = useCallback(async () => {
    ...
    try {
      ...
      const rows = Array.isArray(result?.data) ? result.data : result?.data?.pxResults || result?.pxResults || [];
      setCases(rows);
      setStep("CASE_LIST");
  ...
  const selectCase = useCallback(async (caseRef) => {
    setStep("LOADING");
    setError("");
    setAttachments([]);
    try {
      const response = await fetch(`${API_BASE}/cases/${encodeId(caseRef)}?viewType=page`, { headers: { Authorization: `Bearer ${token}` } });
      const result = await apiResponse(response, "Failed to load case details.");
      ...
      if (assignment?.ID && action?.ID) {
        const metadataResponse = await fetch(`${ASSIGN_BASE}/assignments/${encodeId(assignment.ID)}/actions/${action.ID}?viewType=form`, { headers: { Authorization: `Bearer ${token}` } });
        setIfMatch(metadataResponse.headers.get("If-Match") || metadataResponse.headers.get("ETag") || "");
      }
      setStep("CASE_DETAIL");
    } catch (caught) {
      setError(caught.message);
  ...
  const uploadAttachment = useCallback(async (file) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/attachments/upload`, { method: "POST", headers: { Authorization: `Bearer ${token}` }, body: form });
      const result = await apiResponse(response, "Attachment upload failed.");
      const id = result?.ID || result?.id;
      setAttachments((current) => [...current, { id: id || file.name, name: file.name }]);
  ...
  return (
    <div className="app-shell">
      {step !== "START" && <Header step={step} caseData={caseData} onHome={home} />}
      ...
      {step === "LOADING" && <div className="state-screen"><span className="loader" /><strong>Loading…</strong></div>}
      {step === "ERROR" && (
        <div className="state-screen">
          <div className="error-panel"><strong>Unable to continue</strong><p>{error}</p></div>
          <div className="state-actions"><button className="outline-button" onClick={home}>HOME</button><button className="primary-button" onClick={openQueue}>RETRY</button></div>
        </div>
      )}
      ...
      {step === "SUCCESS" && (
        <div className="state-screen">
          <div className="success-panel"><div className="success-check">✓</div><h1>Submitted</h1><p>The Awaiting Fulfillment assignment has been submitted successfully.</p><strong>{caseData?.businessID || caseData?.ID}</strong></div>
          <div className="state-actions"><button className="outline-button" onClick={() => setStep("CASE_LIST")}>CASE LIST</button><button className="primary-button" onClick={openQueue}>NEW SESSION</button></div>
        </div>
      )}
    </div>
  );
